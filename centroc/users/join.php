...
require_once $abs_us_root . $us_url_root . 'usersc/templates/' . $settings->template . '/navigation.php';
  ...
  if(!Token::check($token)){
    include($abs_us_root.$us_url_root.'usersc/scripts/token_error.php');
        ...
        if (!$agreement_checkbox){
                $validation->addError(["Por favor lee y acepta los terminos y condiciones"]);
        }
        ...
        if($settings->recaptcha == 1 || $settings->recaptcha == 2){
                ...
                if($reCaptchaValid || $settings->recaptcha == 0){
                        ...
                        if($act == 1) {
                                ...
                                $subject = 'Bienvenido al '.$settings->site_name;
